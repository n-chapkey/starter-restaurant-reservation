...
async function validateCapacityForReservation(req, res, next) {
    ...
    if (foundTable.capacity < foundReservation.people) {
        const error = new Error(`table_id ${table_id} does not have sufficient capacity for reservation_id ${reservation_id}.`);
        error.status = 400;
        return next(error);
    }
    return next();
...
async function validateReservationAlreadySeated(req, res, next) {
    ...
    if (foundReservation.status === "seated") {
        const error = new Error(`reservation_id ${reservation_id} is already seated.`);
        error.status = 400;
        return next(error);
    }
    next();
...
module.exports = {
    list,
    create: [hasProperties(...requiredProperties),validateTableName,validateCapacity, asyncErrorBoundary(create)],
    update: [hasProperties(...requiredPropertiesForUpdate), reservationExists, validateCapacityForReservation, validateTableIsOccupied, validateReservationAlreadySeated, asyncErrorBoundary(update)],
    delete: [validateTableId, validateTableIsNotOccupied, asyncErrorBoundary(destroy)],
};